...
const PROJECT_TYPES = [
    ...
    { id: 'backyard', name: 'Whole Backyard', icon: 'https://placehold.co/64x64/transparent/333333?text=🏡&font=roboto' } // Example URL
];
    ...
    return (
        <div className="container mx-auto p-4 md:p-6 max-w-4xl">
            <Card>
                ...
                {/* Location Buckets for Drag and Drop */}
                <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                    <LocationBucket 
                        locationDetails={locations.REGINA} 
                        onDrop={handleDrop} 
                        onDragOver={(e) => handleDragOver(e, locations.REGINA.id)} 
                    ...
                    <LocationBucket 
                        ...
                        onDrop={handleDrop} 
                        onDragOver={(e) => handleDragOver(e, locations.SASKATOON.id)} 
                        onDragLeave={handleDragLeave}
...
const ProjectGridCell = ({ project, locationMap }) => {
    // Component for a single cell in the project display grid
    const locationDetails = Object.values(locationMap).find(loc => loc.id === project.location);
    const tileBgColor = locationDetails ? locationDetails.tileColor : 'bg-gray-100/80 border-gray-400'; // Fallback color
        ...
        <div className={`aspect-square shadow-lg rounded-lg flex flex-col items-center justify-center p-1.5 sm:p-2 text-center border ${tileBgColor} hover:shadow-xl transition-shadow`}>
            {isIconUrl(project.projectIcon) ? (
                ...
                <img 
                    ...
                    className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 object-contain" // Sizes adjusted for larger display
                    onError={(e) => { 
                ...
                />
                // <span className="fallback-icon-text text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl hidden">❓</span>
            ) : (
                // If icon is not a URL (e.g., an emoji), render it as text
            ...
            {/* Display abbreviated location name */}
...
const DisplayPage = () => {
    // Component for the main display board (optimized for vertical 4K TV)
    ...
    // Determine projects to display (up to monthly goal)
    ...
    // Heuristic for number of columns on the display grid, aiming for good fit on vertical 4K
    const numColumns = Math.min(6, Math.max(3, Math.ceil(Math.sqrt(monthlyGoal * 0.8)))); // Adjust as needed
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white p-4 sm:p-6 md:p-8 flex flex-col items-center justify-center">
            {/* Container to manage layout on large vertical screen */}
            <div className="w-full max-w-[2100px] h-full flex flex-col"> {/* Max width can be adjusted based on 4K TV aspect ratio */}
                ...
                {/* Main content area for the project grid */}
                ...
                {/* Footer with navigation and admin buttons */}
...
// --- Main App Component ---
function App() {
    const { currentPage, setCurrentPage: contextSetCurrentPage } = useContext(AppContext); // Get setCurrentPage from context

    // Effect to handle URL hash changes for navigation
    useEffect(() => {
        const handleHashChange = () => {
            const hash = window.location.hash;
            // Use setCurrentPage from context to update app state based on hash
            if (contextSetCurrentPage) {
                 if (hash === '#/display') {
                    contextSetCurrentPage('display');
                } else if (hash === '#/input' || hash === '') { // Treat empty hash as input page
                    contextSetCurrentPage('input');
                }
            }
        };

        window.addEventListener('hashchange', handleHashChange, false);
        handleHashChange(); // Call once on load to set initial page based on hash

        // Cleanup event listener
        return () => {
            window.removeEventListener('hashchange', handleHashChange, false);
        };
    }, [contextSetCurrentPage]); // Add contextSetCurrentPage to dependency array to ensure it's available

    // Render either InputPage or DisplayPage based on currentPage state
    return (
...
// Export the App component wrapped in its provider
